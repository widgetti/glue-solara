...
def LinkStep(
    ...
    def save_and_next():
        ...
        print("linking", suggested_links.value)
        step.value += 1
    ...
    print("result state link:", result.state)
    ...
    if result.state == solara.ResultState.RUNNING:
        ...
        with solara.Row(justify="end"):
            solara.Button("Next", on_click=save_and_next, color="primary")
...
def RenameStep(
    ...
    openai = solara.use_memo(lambda: OpenAI(), dependencies=[], debug_name="openai_rename")
    openai.api_key = os.getenv("OPENAI_API_KEY")
    ...
    def save_and_finish():
        for item in suggested_renames.value:
            if item["is_chosen"]:
                dataset_name = list(item["rename"][0].keys())[0]
                data_index = list(list_of_columns.keys()).index(dataset_name)
                # Find index of the component we are looking at
                data_component_index = list_of_columns[dataset_name].index(
                    list(item["rename"][0].values())[0]
                )
                print("data:", data_collection[data_index][data_component_index])
                print("new name:", list(item["rename"][1].values())[0])
                # data_collection[data_index][data_component_index] = list(
                #     item["rename"][1].values()
                # )[0]
    ...
    message = f"""Here are some datasets with the names of columns present in each dataset: {list_of_columns}.
        Could you suggest new descriptive names for columns, where it would improve the name quality.
        For example, 'RAJ2000' could be renamed to 'Right Ascension'."""
    ...
    if result.state == solara.ResultState.RUNNING:
        ...
        with solara.v.List():
            if len(suggested_renames.value) == 0:
                with solara.v.ListItem():
                    with solara.v.ListItemContent():
                        solara.HTML(tag="h2", unsafe_innerHTML="No new names suggested")
            for index, item in enumerate(suggested_renames.value):

                def set_link_chosen(new_value, index=index):
                    suggested_renames.value[index]["is_chosen"] = new_value
                ...
                with solara.v.ListItem():
                    with solara.v.ListItemContent():
                        with solara.Row(style={"align-items": "center"}, justify="space-between"):
                            solara.HTML(
                                tag="h2",
                                unsafe_innerHTML=item["rename"][0],
                            )
                            with solara.ToggleButtonsSingle(
                                mandatory=False,
                                value=item["is_chosen"],
                                ...
                                solara.Button(
                                    value=True,
                                    icon_name="mdi-rename",
                                    color="primary",
                                    text=True,
                                )
                            solara.HTML(
                                tag="h2",
                                unsafe_innerHTML=item["rename"][1],
                            )
        with solara.Row(justify="end"):
            solara.Button("Next", on_click=save_and_finish, color="primary")
    ...
    with solara.v.Dialog(
        ...
        with solara.v.Stepper(v_model=current_step.value):
            with solara.v.StepperHeader():
                ...
                solara.v.Divider()
                with solara.v.StepperStep(step=2, complete=current_step.value > 2):
                    solara.HTML(tag="h2", unsafe_innerHTML="Rename Columns")
